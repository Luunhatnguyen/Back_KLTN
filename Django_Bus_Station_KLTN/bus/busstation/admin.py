    ...
    def list_carrier(self, request):
        ...
        for key in request.POST.keys():
            if key.startswith('APPROVE:'):
                action = key[8:]
                print(action)
                carrierUpdate = Carrier.objects.get(id=action)
                carrierUpdate.status_code = "APPROVE"
                carrierUpdate.save()
                print("save APPROVE")
                break
            if key.startswith('NOT APPROVE:'):
                action = key[12:]
                print(action)
                carrierUpdate = Carrier.objects.get(id=action)
                carrierUpdate.status_code = "NOT APPROVE"
                carrierUpdate.save()
                print("save NOT APPROVE")
                break
            if key.startswith('TERMINATE:'):
                action = key[10:]
                print(action)
                carrierUpdate = Carrier.objects.get(id=action)
                carrierUpdate.status_code = "TERMINATE"
                carrierUpdate.save()
                print("save TERMINATE")
                break
...
class UserAdmin(admin.ModelAdmin):
    ...
    def image_view(self, user):
        return mark_safe(
            "<img src='/static/{url}' alt='test' width='120px' />".format(url=user.avatar.name)
        )
