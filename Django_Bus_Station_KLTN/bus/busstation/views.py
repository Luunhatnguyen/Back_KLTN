...
class ViewCarrierViewSet(viewsets.ViewSet, generics.CreateAPIView, generics.ListAPIView, generics.UpdateAPIView,
                  generics.RetrieveAPIView):
    queryset = Carrier.objects.filter(active=True)
    serializer_class = CarrierViewSerializer

    @action(methods=['get'], detail=True, url_path='')
    def get_carrier_by_userID(self, request, pk):
        user = User.objects.get(id=pk)
        userId = Carrier.objects.filter(userID=user, active=True)

        return Response(data=CarrierViewSerializer(userId, many=True,
...
class CarrierViewSet(viewsets.ViewSet, generics.CreateAPIView, generics.ListAPIView, generics.UpdateAPIView,
    ...
    # def approve_carrier(self, request, pk):
    #     carrier = self.get_object()
    #     carrier.status_code = "APPROVE"
    #     carrier.save()
    #     return Response({'status': 'Carrier submited successfully!'}, status=status.HTTP_200_OK)
...
class BusViewSet(viewsets.ViewSet, generics.CreateAPIView, generics.ListAPIView, generics.UpdateAPIView,
    ...
    def get_comments(self, request, pk):
        buss = self.get_object()
        comments = buss.comments.select_related('user')
...
class BusPostViewSet(viewsets.ViewSet, generics.CreateAPIView):
    queryset = Bus.objects.filter(active=True)
    serializer_class = BusPostSerializer


class HireOptionViewSet(viewsets.ViewSet, generics.CreateAPIView, generics.ListAPIView, generics.UpdateAPIView,
                    generics.RetrieveAPIView):
    queryset = HireOption.objects.filter(active=True)
    serializer_class = HireOptionSerializer
...
class BusRouteViewSet(viewsets.ViewSet, generics.CreateAPIView, generics.ListAPIView, generics.UpdateAPIView,
    ...
    def get_route_by_bus(self, request, pk):
        ...
        busId = BusRoute.objects.filter(busID_id=bus, active=True)
